...
export class GameEngine {
  ...
  private resolveActions(player1Action: Action | null, player2Action: Action | null): void {
    ...
    const player1 = this.state.players.find(p => p.playerId === 'player1-id');
    const player2 = this.state.players.find(p => p.playerId === 'npc-player-id'); // Assuming player2 is NPC
  ...
  private checkWinCondition(): void {
    ...
    this.state.players.forEach(player => {
      if (player.properties <= 0) {
        console.log(`Player ${player.playerId} has lost all properties! Game Over.`);
        // TODO: Implement actual game over state
      }
    });
  }
}
