...
また、これらのカードとは別に、手札のカード提出の代わりとして使える「資金集め」のコマンドがあります。
...
### ユーザー認証機能の導入
- プレイヤーごとのアカウント作成、デッキの保存、対戦履歴の管理などを可能にし、よりパーソナライズされたゲーム体験を提供します。
### ゲームロジックのサーバーサイド実行（Server-Authoritative Game Engine）
- オンラインPvPモードの公平性とセキュリティを確保するため、ゲームのコアロジックをサーバー側で実行する設計に移行します。これにより、チート行為を防止し、信頼性の高い対戦環境を構築します。
...
- **Python + FastAPI**
    ...
    - ゲームロジック: **初期のNPC対戦ロジックはクライアント側で実行されますが、将来的なPvPやチート対策のため、ゲームのコアロジックはサーバーサイド（Python）で一元管理される設計となっています。**

### インフラ/ホスティング
- **Firebase**
    - ホスティング: フロントエンドの静的ファイルを**Firebase Hosting**で配信します。
    - バックエンド: FastAPIで構築したバックエンドは、**Cloud Functions for Firebase**（または将来的にCloud Run）にデプロイされます。
    - データベース: ユーザーデータや対戦履歴、ゲームの状態同期には**Firebase Realtime Database**を初期段階で利用します。（将来的にFirestoreへの移行も検討）
...
- **デプロイ**:
    ...
    - バックエンド: FastAPIサーバーは、**Cloud Functions for Firebase**（または将来的にWebSocketサーバーが必要な場合はCloud Runなど）にデプロイされます。
...
- `/src/game`: Phaser 3のゲームロジックをすべてここに集約します。
    ...
    - `effects`: カード効果の具体的な実装を定義します。（例: `acquireProperty.ts`, `gainFunds.ts`）
    - `ai`: NPCの思考ロジックを実装します。
    - `engine.ts`: ターンの進行、カードの効果処理、勝利判定など、ゲームの根幹をなすロジックを担います。
    - `PhaserGame.ts`: Phaserゲームインスタンスの初期化と管理を行います。
- `/src/auth`: 認証に関するロジック（AuthAdapterインターフェース、NullAuthAdapterなど）を管理します。
- `/src/api`: バックエンドとの通信に関するロジックをカプセル化します。
- `/src/hooks`: カスタムReactフックを配置します。（例: `useLocalStorage.ts`）
- `/src/types.ts`: ゲーム全体で共有されるTypeScriptの型定義を管理します。

### 9.2. バックエンド (api-server)
- `/app/api`: クライアントからのAPIリクエストを受け付けるエンドポイントを定義します。（例: `deck_endpoints.py`, `game_endpoints.py`）
- `/app/game`: ゲームのコアロジックを管理するディレクトリです。
    ...
    - `cards`: 個々のカード効果の具体的な実装を定義します。（例: `acquire.py`, `defend.py`）
- `/app/db/database.py`: データベースへの接続や、データの読み書きといった低レベルな処理を集約します。
- `/app/main.py`: FastAPIアプリケーションのエントリーポイントです。

---
    1 ## 10. NPC対戦の設計
    2 本ゲームでは、プレイヤーが一人でも楽しめるよう、NPCとの対戦機能を実装します。
    3
    4 - **実装方針**:
    5     - **初期段階では、NPCの思考ロジックはクライアント側で動作する重み付け行動型のAIとして実装されます。**
      将来的には、サーバーサイドでのAI実行も視野に入れています。
    6     -
      NPCは、ゲームの状況（自分の資金、相手の不動産の数など）に応じて、手札の各カードに動的な重み付けを行います。
    7     - 設定された重み付けに基づき、確率的にプレイするカードを決定します。
    8     -
      これにより、単純なランダム行動ではなく、ある程度の戦略性を持ったNPCとの対戦が可能となり、プレイヤーは段階的
      にゲームの読み合いを学べます。
    9
   10 - **NPCキャラクターの多様化と管理**:
   11     - **将来的には、複数のNPCキャラクターを用意し、それぞれ異なる戦略や難易度を提供します。**
   12     - 各NPCの個性は、**行動の重み付けパラメータ**（カード選択の傾向など）や、**使用するデッキ**
      を変動させることで表現されます。
   13     - これらのNPC設定データ（重み付け、デッキIDなど）は、外部ファイル（JSON）またはFirebase Realtime
      Databaseなどのデータストアで管理され、コードの変更なしにNPCの挙動を調整・追加できるデータ駆動型の設計としま
      す。
